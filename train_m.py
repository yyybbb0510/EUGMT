...
def get_args(known=False):
    ...
    parser.add_argument('--data_path', type=str, default='./dataset/Her2', help='path to the data')
    ...
    parser.add_argument('--log_freq', type=float, default=100, help='logging frequency of metrics accord to the current iteration')
...
def main(is_debug=False):
    ...
    # Project Saving Path
    project_path = args.project + '_{}_labeledTCT_{}/'.format(args.backbone, args.labeled_percentage)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            with torch.no_grad():
                nimage_ng = uimage
                output = teacher(nimage_ng)['out']
                output_soft = torch.softmax(output, dim=1)
                noisy_output = teacher(noisy_input)['out']

            #s1
            outputs_onehot_np1 = pseudo1.cpu().detach().numpy()
            t_output_soft_np = output_soft.cpu().detach().numpy()
            t_output_soft_np_accumulated = np.swapaxes(t_output_soft_np, 1, 2)
            t_output_soft_np_accumulated = np.swapaxes(t_output_soft_np_accumulated, 2, 3)
            t_output_soft_np_accumulated = t_output_soft_np_accumulated.reshape(-1, num_classes)
            t_output_soft_np_accumulated = np.ascontiguousarray(t_output_soft_np_accumulated)
            
            outputs_onehot_np_accumulated1 = outputs_onehot_np1.reshape(-1).astype(np.uint8)

            #s2
            outputs_onehot_np2 = pseudo2.cpu().detach().numpy()
            outputs_onehot_np_accumulated2 = outputs_onehot_np2.reshape(-1).astype(np.uint8)
            consistency_weight = get_current_consistency_weight(epoch)
            consistency_dist1 = softmax_mse_loss(pred1_u, noisy_output)
            consistency_dist2 = softmax_mse_loss(pred2_u, noisy_output)

            #s1
            noise1 = noise
            noise1 = cleanlab.filter.find_label_issues(outputs_onehot_np_accumulated1, t_output_soft_np_accumulated, filter_by='both', n_jobs=1)
            confident_maps_np1 = noise1.reshape(-1, patch_size[0], patch_size[1]).astype(np.uint8)
            confident_maps1 = torch.from_numpy(confident_maps_np1).cuda(pred1_u_feature.device.index)
            mask1 = (confident_maps1 == 1).float() 
            mask1 = torch.unsqueeze(mask1, 1) 
            mask1 = torch.cat((mask1, mask1), 1)
            consistency_loss1 = torch.sum(mask1*consistency_dist1)/(torch.sum(mask1)+1e-16)
            consistencyloss1 = consistency_weight * consistency_loss1

            #s2
            noise2 = noise
            noise2 = cleanlab.filter.find_label_issues(outputs_onehot_np_accumulated2, t_output_soft_np_accumulated, filter_by='both', n_jobs=1)
            confident_maps_np2 = noise2.reshape(-1, patch_size[0], patch_size[1]).astype(np.uint8)
            confident_maps2 = torch.from_numpy(confident_maps_np2).cuda(pred2_u_feature.device.index)
            mask2 = (confident_maps2 == 1).float() 
            mask2 = torch.unsqueeze(mask2, 1) 
            mask2 = torch.cat((mask2, mask2), 1)
            consistency_loss2 = torch.sum(mask2*consistency_dist2)/(torch.sum(mask2)+1e-16)
            consistencyloss2 = consistency_weight * consistency_loss2
            cons_loss = consistencyloss1 + consistencyloss2
            ...
            loss = loss_sup + lambda_ * loss_u + cons_loss
            loss.backward()
            optimizer1.step()
            optimizer2.step()
            ...
            with torch.no_grad():
                ...
                # noise_2 = torch.clamp(torch.randn_like(uimage2) * 0.1, -0.2, 0.2)
                # noisy_input = uimage2 + noise_2

  
            # Re-Estimate the pseudo-labels on the new uimages
            pred1_u = student1(uimage2)['out']
            pred2_u = student2(uimage2)['out']
            # pred1_u_soft = torch.softmax(pred1_u, dim=1)
            # pred1_u_onehot = torch.argmax(pred1_u_soft, dim=1)
            # pred2_u_soft = torch.softmax(pred2_u, dim=1)
            # pred2_u_onehot = torch.argmax(pred2_u_soft, dim=1)
            # with torch.no_grad():
                # output = teacher(uimage2)['out']
                # output_soft = torch.softmax(output, dim=1)
                # noisy_output = teacher(noisy_input)['out']
                # noisy_output_soft = torch.softmax(noisy_output, dim=1)
            #s1
            # outputs_onehot_np1 = pred1_u_onehot.cpu().detach().numpy()
            # t_output_soft_np = output_soft.cpu().detach().numpy()
            # t_output_soft_np_accumulated = np.swapaxes(t_output_soft_np, 1, 2)
            # t_output_soft_np_accumulated = np.swapaxes(t_output_soft_np_accumulated, 2, 3)
            # t_output_soft_np_accumulated = t_output_soft_np_accumulated.reshape(-1, num_classes)
            ...
            #s2
            # outputs_onehot_np2 = pred2_u_onehot.cpu().detach().numpy()
            # outputs_onehot_np_accumulated2 = outputs_onehot_np2.reshape(-1).astype(np.uint8)
            ...
            # noise1 = cleanlab.filter.find_label_issues(outputs_onehot_np_accumulated1, t_output_soft_np_accumulated, filter_by='both', n_jobs=1)
            # confident_maps_np1 = noise1.reshape(-1, patch_size[0], patch_size[1]).astype(np.uint8)
            # confident_maps1 = torch.from_numpy(confident_maps_np1).cuda(pred1_u_soft.device.index)
            ...
            # noise2 = cleanlab.filter.find_label_issues(outputs_onehot_np_accumulated2, t_output_soft_np_accumulated, filter_by='both', n_jobs=1)
            # confident_maps_np2 = noise2.reshape(-1, patch_size[0], patch_size[1]).astype(np.uint8)
            # confident_maps2 = torch.from_numpy(confident_maps_np2).cuda(pred2_u_soft.device.index)
            # mask2 = (confident_maps2 == 1).float() 
            # mask2 = torch.unsqueeze(mask2, 1) 
            # mask2 = torch.cat((mask2, mask2), 1)
            # consistency_loss2 = torch.sum(mask2*consistency_dist2)/(torch.sum(mask2)+1e-16)
            ...
            # cons_loss_2 = consistencyloss1 + consistencyloss2
